...
function appIsExistAll(callbackAll) {
    ...
    var claimArr = [];
    api.ajax({
        url: apiUrl + '/api/services/app/WorkFlow/GetUnconFirmedTasks?Lng=' + CurrentLocation.lon + '&Lat=' + CurrentLocation.lat + '&Account=' + $api.getStorage("thirdPartyAccount") + '&PassWord=' + $api.getStorage("thirdPartyPassWord") + '&PageIndex=1&MaxResultCount=1000',
        method: 'get',
        timeout: 15,
        dataType: 'json',
        headers: headers,
        ...
        if (ret.status) {
            ...
            var body = {
                body: JSON.stringify({
                    ...
                    account: $api.getStorage('thirdPartyAccount')!=undefined?$api.getStorage('thirdPartyAccount'):'',
                    passWord: $api.getStorage('thirdPartyPassWord')!=undefined?$api.getStorage('thirdPartyPassWord'):''
                })
            };
                ...
                if (ret.status) {
                    if (ret.data.length == 0) {
                        db.executeSql({
                          ...
                          // console.log("insert"+JSON.stringify(err));
                            if (ret.status) {
...
function dataQuery(fromCb, callback) { //fromCb 表示不是抄表管家更新基础数据 ，true表示是更新抄表管家基础数据
    ...
    if (fromCb) {
        ...
        var ret = db.selectSqlSync({
            name: 'Wsdatabase',
            sql: 'select * from POSTING_LIST where userName ="' + CurentUserName + '" and NotSave="1" and NotUploader="0" and Files=""'
        });
        if (ret.status) {
            if (ret.data.length > 0) {
                returnRet.appName = '表务管理';
...
function onCommentGetClassify(callback) {
    var CurrentLocation = $api.getStorage('CurrentLocation');
    var thirdPartyAccount = $api.getStorage('thirdPartyAccount');
    var thirdPartyPassWord = $api.getStorage('thirdPartyPassWord');
    ...
    var apipath = '';
          if(addressHistory.intranetObj.url!=""){
            if (apiUrl == addressHistory.intranetObj.url) { //内网
              if(addressHistory.extranetObj.url !="" && addressHistory.extranetObj.url!='' && addressHistory.extranetObj.url!=null){
                  $api.setStorage('apiUrl', addressHistory.extranetObj.url);
              }
            }
          }
          if(addressHistory.extranetObj.url!=""){
            if (apiUrl == addressHistory.extranetObj.url) { //外网
              if(addressHistory.intranetObj.url!="" && addressHistory.intranetObj.url!='' && addressHistory.intranetObj.url!=null){
                  $api.setStorage('apiUrl', addressHistory.intranetObj.url);
              }
              // return apipath;
            }
          }
...
/**
* logo更新
* 判断，如果用户已经登录过，要判断logo是否存在， 没有的话，就要重新下载，有下载过，你要设置一个时间，比如7天的时间，更新一下logo
*/
function updateLogo(){
  // this.userName
    // console.log(JSON.stringify($api.getStorage('userLoginInformation')));
    if($api.getStorage('userLoginInformation')){
        var fs = api.require("fs");
        var logoret = fs.readDirSync({
            path: ''+api.fsDir+'/unarchive/logo'
        });
        if (logoret.status) {
            var imgret = fs.readDirSync({
                path: ''+api.fsDir+'/unarchive/logo/'+logoret.data[0]
            });
            if (imgret.status) {
              // 判断 时间是否存在 7天更新
              if($api.getStorage('logoTime')){
                var dayBet = getDaysBetween($api.getStorage('logoTime'),new Date().getTime());
                console.log(dayBet);
                if(parseInt(dayBet)%7 == 0){
                  downloadLogo();
                }
              }else{
                $api.setStorage('logoTime', new Date().getTime());
              }
            }else {
              downloadLogo()
            }
...
function logoShow(){
    ...
    if (logoret.status) {
        ...
        if (imgret.status) {
          $(".logo").css("background-image",'url('+api.fsDir+'/unarchive/logo/'+logoret.data [0]+'/'+imgret.data[0]+')');
        }else {
